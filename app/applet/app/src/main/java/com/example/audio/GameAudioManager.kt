...
enum class SoundEffect(val resName: String, val cooldownMs: Long = 80L) {
    ...
    LIFE_LOST("mismatch", 120L),
    GAME_OVER("mismatch", 300L),
...
class GameAudioManager private constructor(private val context: Context) {
    ...
    private fun loadSoundEffects() {
        for (effect in SoundEffect.values()) {
            val resId = context.resources.getIdentifier(effect.resName, "raw", context.packageName)
            if (resId != 0) {
                try {
                    val soundId = soundPool.load(context, resId, 1)
                    soundMap[effect] = soundId
                    Log.d(TAG, "loadSoundEffects: effect=${effect.name}, resName=${effect.resName}, resId=$resId -> soundId=$soundId")
                } catch (e: Exception) {
                    Log.w(TAG, "loadSoundEffects: Falha ao carregar SFX ${effect.resName}", e)
                }
            } else {
